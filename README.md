...
`collect` and `run` accept `--progress` to draw a single-line progress bar on stderr during collection (explicit opt-in; off by default so CI logs stay line-based). It redraws in place with `\r` and no ANSI escapes, and log lines erase the bar before printing so the two never collide. Failed requests are also surfaced during collection at the default level: the first occurrence of each `error_kind` prints a `WARN` with the endpoint's own error message (secrets redacted), and the collection summary ends with a per-kind failure breakdown plus troubleshooting hints.
