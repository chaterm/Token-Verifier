...
`error_kind` 不只是落盘字段，它必须**当场**到达用户：一轮采集是上百个请求、
要联网花钱，密钥写错这类问题若等到整轮跑完才暴露，钱已经花掉了。

因此每种 `error_kind` 的**首例**在发生时就打一条 `WARN`（默认日志级别可见），
带 `http_code`、探针、协议与端点返回的原话；同类错误后续不再逐条打印，避免
131 个失败刷出 131 行。采集结束后再按分类印一次分布汇总，附**排查方向**
（401/403 → 检查 `target.api_key_env`；429 → 调 `runtime.min_interval_ms`；
`timeout` → 调 `runtime.timeout_sec`，等等）。零成功样本时额外显式告警 ——
那份 rawData 拿去 compare 不会有有意义的结论。

端点返回的文本在写终端前一律过 `logging.SanitizeTerminal` 剥掉 C0 控制字符：
恶意端点可以用 ANSI 转义序列清屏、改色或用 CR 覆盖已打印的行，伪造出
「采集成功」的假象。**落盘的 `error_detail` 保留原始字节**（仅做密钥脱敏与
4KiB 截断）—— 终端安全属于渲染层，证据本身不被渲染层改写。
