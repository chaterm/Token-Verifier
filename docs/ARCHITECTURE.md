...
| 存储、服务、Web UI、TUI | 无状态 CLI，输出只有 stdout / JSON / JUnit。`--progress` 的单行进度条不是 TUI：它只写 stderr、`\r` 原地重绘、不含 ANSI 转义，默认关闭且由用户显式开启，机器消费的输出不受任何影响 |
...
**阈值一律由使用者提供，工具不带任何默认值。** 缺少任一待比较探针
（或其任一档位）的阈值即拒绝执行并退出（`exit 2`），指名缺哪个。设置方式：
命令行 `--threshold` 或配置文件 `thresholds` 段，命令行优先；两级键
`<probe>` / `<probe>.<bucket>` 支持按上下文档位细化，档位级覆盖探针级兜底。
报告的每个探针行（多档位时含档位子行）都印出实际取值与来源（`config` / `flag`）。

比较按 `context_bucket` 分区：每档位独立判定，探针级结论取最差档位。
这让「短上下文正常、长上下文回归」不再被跨档位聚合稀释 —— `context_bucket`
既然是所有探针共享的采集轴（§5），判据轴与它对齐是自然推论；代价是
p 值类探针的单档位检验功效下降，见 SPEC-CONFIG §7。
