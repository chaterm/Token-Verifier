...
五个探针，每个产出一个统计量和一个判定（机理见 [PROBES.md](./docs/PROBES.md)）：

| 探针 | 观测什么 | 统计量 | 能抓到什么 |
| :--- | :--- | :--- | :--- |
| **`onetoken`** | 温度 1.0 下极短答案的取值分布 | 分布间 Jensen–Shannon 散度 | 换模型、换量化、采样参数被改 |
| **`tokenizer`** | 分词器敏感输入上服务端上报的输入 token 数 | 逐题整数相等性 → 列联表卡方 | 分词器/模型家族更换、中间层悄悄改写你的 prompt |
| **`needle`** | 长上下文中按题目声明位置埋入的确定性派生标记的召回（一个埋点 = 一个统计单元） | 埋点粒度命中数 → 2×2 卡方 | 上下文被静默截断、长上下文能力降级、窗口大小造假 |
| **`think-effort`** | 指定思考强度下的思考量分布（token 数；协议无思考 token 字段时用思考文本字数） | 逐 cell（强度 × 协议）Mann-Whitney U → Fisher 合并 p 值 | 思考被关闭、预算被削减、强度参数被忽略、声称思考量与实交文本不符 |
| **`toolcall`** | 工具选择、参数语法合法性、并行调用 | 选择分布 JSD + 池化合法率卡方（取更差分量判定） | agent 行为漂移、工具调用能力降级、声称支持但实现质量不同 |

**请求格式（协议）与上下文长度是正交于探针的两个测试轴，不是探针本身。** 每个探针都会在你声明的所有协议（`openai-chat` / `anthropic-messages` / `openai-responses`，流式与非流式）和所有上下文档位上各跑一遍，判定按 `(探针, 协议, 上下文档位)` 的切片独立做出。这带来一个直接后果：端点若只在某一种请求格式下、或只在某一段上下文长度上做了替换/截断/降级，它会在对应切片上单独露出来，而不会被其它正常的切片稀释成「整体上没问题」。换句话说，**跨格式、跨长度的定向造假也是可被发现的**，不是只在「整体看」时才显形。
